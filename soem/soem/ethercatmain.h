...
{
   uint32  LogStart;	//Ls： log起点
   uint16  LogLength;	//Ll： log长度
   uint8   LogStartbit;	//Lsb：log开始位置
   uint8   LogEndbit;	//Leb：log结束位置
   uint16  PhysStart;	//Ps： 物理起点
   uint8   PhysStartBit;//Psb：物理开始位置
   uint8   FMMUtype;	//Ty： 类型:输入输出
...
typedef struct ec_slave
{
   ...
   /** output bits */
   uint16           Obits;
   /** output bytes, if Obits < 8 then Obytes = 0  --> output size*/
   uint32           Obytes;
   /** output pointer in IOmap buffer		--> output address*/
   uint8            *outputs;
   /** startbit in first output byte */
   uint8            Ostartbit;
   /** input bits */
   uint16           Ibits;
   /** input bytes, if Ibits < 8 then Ibytes = 0 --> input size*/
   uint32           Ibytes;
   /** input pointer in IOmap buffer		--> input address*/
   uint8            *inputs;
