...
boolean ecx_configdc(ecx_contextt *context)
{
   ...
   /*在计算时间偏差Toffset(n)和传输延时之前，主站需要发送一个广播命令BWR，写所有从站端口0的接收时间寄存器0x0900，
    *将所有从站捕捉数据帧第一个前导位到达每个端口的本地时间保存到寄存器0x0900~0x090F, 每个端口使用4个字节。*/
   ecx_BWR(context->port, 0, ECT_REG_DCTIME0, sizeof(ht), &ht, EC_TIMEOUTRET);  /* latch DCrecvTimeA of all slaves */
   mastertime = osal_current_time();//可以看到获取到的主站时间是ms级的
   ...
   for (i = 1; i <= *(context->slavecount); i++)
   {
      ...
      if (context->slavelist[i].hasdc)
      {
		  ...
		  *注意：SOEM只在初始化时使从站时间同步主站时间，若要运行后使主从时间同步，需要定期修正主站时间或从站时间。*/
