...
int getAnalogValueImpl(int deviceId, int channelId) {
	...
	//TODO: need to be more flexible
		...
		int value = dsersor->counterValue - 0x7fff - 1;	//解决返回值从uint32转为int类型问题
		return value;
	}
	return INF;
...
void CALLBACK record(UINT uID, UINT uMsg, DWORD dwUser, DWORD dw1, DWORD dw2) {
	...
	if (Localptr > 1000) {
		Localptr = 0;
		cout << "开始时间" << getSystemTime() << endl;
		osal_thread_create(&thread, NULL, localCallback, result);
		cout << "结束时间" << getSystemTime() << endl;
	}
