...
//************************************
// Method:    检测并返回从站数量
// FullName:  ecx_detect_slaves
// Access:    public 
// Returns:   如果成功返回从站数量，从站个数超过最大允许数量则返回-2
// Qualifier:
// Parameter: 返回EtherCat上下文context
//************************************
int ecx_detect_slaves(ecx_contextt *context)
{
   ...
   /* Reset all slaves to Init *//* netX100 should now be happy 注：netX100是一种高度集成的网络控制器*/
   ecx_BWR(context->port, 0x0000, ECT_REG_ALCTL, sizeof(b), &b, EC_TIMEOUTRET3);       
   ecx_BWR(context->port, 0x0000, ECT_REG_ALCTL, sizeof(b), &b, EC_TIMEOUTRET3);       
...
int ecx_config_init(ecx_contextt *context, uint8 usetable)
{
   ...
   if (wkc > 0)
   {
      ...
      {
         ADPh = (uint16)(1 - slave);
         val16 = ecx_APRDw(context->port, ADPh, ECT_REG_PDICTL, EC_TIMEOUTRET3); /* 读取slave的interface */
