 ...
 * EtherCAT报文单元，广播，自增，配置以及逻辑地址数据转换。
 * 所有的转换都是blocking（阻塞式）的，所以要等待该结构返回master或是超时。
 * 如果这是无法接受的，那你就构建你自己的数据帧去吧，用nicdrv.c里面的函数就行。
 */
...
static void ecx_writedatagramdata(void *datagramdata, ec_cmdtype com, uint16 length, const void * data)
{
   if (length > 0)
   {
      switch (com)
      {
         ...
         case EC_CMD_BRD:
            /* Fall-through 失败*/
         case EC_CMD_LRD://LRD：logical memory read
...
/** BRW "broadcast write" primitive. Blocking.	广播写
 *
 * @param[in] port        = port context struct
...
/** BRD "broadcast read" primitive. Blocking.	广播读
 *
 * @param[in] port        = port context struct
 * @param[in]  ADP        = Address Position, normally 0
 * @param[in]  ADO        = Address Offset, slave memory address
...
/** APRD "auto increment address read" primitive. Blocking.	自增地址读，阻塞式
 *
 * @param[in] port        = port context struct
...
/** APRMW "auto increment address read, multiple write" primitive. Blocking.自增地址读，多次写入， 阻塞式
 *
 * @param[in] port        = port context struct
...
/** FPRMW "configured address read, multiple write" primitive. Blocking.指定地址读，多次写入，阻塞式
 *
 * @param[in] port        = port context struct
...
/** APRDw "auto increment address read" word return primitive. Blocking.自增地址读，返回word
 *
 * @param[in] port        = port context struct
 * @param[in] ADP         = Address Position, each slave ++, slave that has 0 reads.
 * @param[in] ADO         = Address Offset, slave memory address
...
/** FPRD "configured address read" primitive. Blocking.		配置地址读
 *
 * @param[in] port        = port context struct
 * @param[in]  ADP        = Address Position, slave that has address reads.
 * @param[in]  ADO        = Address Offset, slave memory address
...
/** FPRDw "configured address read" word return primitive. Blocking.配置地址读 word返回
 *
 * @param[in] port        = port context struct
 * @param[in] ADP         = Address Position, slave that has address reads.
 * @param[in] ADO         = Address Offset, slave memory address
...
/** APWR "auto increment address write" primitive. Blocking.	自增地址写
 *
 * @param[in] port        = port context struct
...
/** APWRw "auto increment address write" word primitive. Blocking.	自增地址写 word返回
 *
 * @param[in] port        = port context struct
 * @param[in] ADP         = Address Position, each slave ++, slave that has 0 writes.
 * @param[in] ADO         = Address Offset, slave memory address
...
/** FPWR "configured address write" primitive. Blocking.	配置地址写
 *
 * @param[in] port        = port context struct
...
/** FPWRw "configured address write" word primitive. Blocking.	配置地址写 word返回
 *
 * @param[in] port        = port context struct
 * @param[in] ADP         = Address Position, slave that has address writes.
 * @param[in] ADO         = Address Offset, slave memory address
...
/** LRW "logical memory read / write" primitive. Blocking.		逻辑地址读写
 *
 * @param[in] port        = port context struct
 * @param[in]     LogAdr  = Logical memory address
 * @param[in]     length  = length of databuffer
 * @param[in,out] data    = databuffer to write and to read from slave. 数据有读有写
 * @param[in]     timeout = timeout in us, standard is EC_TIMEOUTRET
 * @return Workcounter or EC_NOFRAME
 */
...
/** LRD "logical memory read" primitive. Blocking.		逻辑地址读
 *
 * @param[in] port        = port context struct
 * @param[in]  LogAdr     = Logical memory address
...
/** LWR "logical memory write" primitive. Blocking.		逻辑地址写
 *
 * @param[in] port        = port context struct
 * @param[in] LogAdr      = Logical memory address
 * @param[in] length      = length of databuffer
...
/** LRW "logical memory read / write" primitive plus Clock Distribution. Blocking.	逻辑地址读写 带分布式时钟版
